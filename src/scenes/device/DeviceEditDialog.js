...
const checkoutSchema = yup.object().shape({
  ...
  sms: yup.string().required('Required scam messager'),
...
const DeviceEditDialog = ({ deviceId, open, onClose, loadDevices }) => {
  ...
  const fetchDeviceData = async (deviceId) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8083/api/v1/device/${deviceId}`, config);
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(`http://localhost:8083/api/v1/device/${deviceId}`, editedData, config);
