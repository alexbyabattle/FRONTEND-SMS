...
const DeviceTable = () => {
  ...
  const loadDevices = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8083/api/v1/device/list`, {
        headers: {
  ...
  const columns = [
...

    {
      ...
      renderCell: ({ row }) => {
        ...
        if ([  "Not_spam",  "APPROVED"].includes(row.status)) {
          statusColor = "#4CAF50"; // Green for success statuses
          textColor = "#FFFFFF"; // Text color white for success statuses
        } else if (["Spam", "FAULT", "SOLUTION_PENDING", "SCAM"].includes(row.status)) {
